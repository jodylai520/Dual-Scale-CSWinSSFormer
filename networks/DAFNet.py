...
from .blocks import DualAttentionFusionBlock,SymmetricFusionPatchMerge
...
class EncoderFusionStage(nn.Module):
    def __init__(self, dim_in, dim_out, res, split_size_h, split_size_l,
        ...
        self.shift_patch_merging = SymmetricFusionPatchMerge(dim_in,res)
        res = res//2
        self.dual_fusion = []
        for i in range(depth):
            self.dual_fusion.append(DualAttentionFusionBlock(dim_out//2,res,split_size_h,split_size_l,num_heads_h,num_heads_l,
...
class FirstFusionStage(nn.Module):
    def __init__(self, dim_in, embed_dim, res, split_size_h, split_size_l,
        ...
        self.dual_fusion = []
        for i in range(depth):
            self.dual_fusion.append(DualAttentionFusionBlock(dim,res,split_size_h,split_size_l,num_heads_h,num_heads_l,
                                     qkv_bias,qk_scale,act_layer,norm_layer,last_stage))
...
#         super().__init__()
#         # print(dim_in, res)
#         self.dual_fusion = DualAttentionFusionBlock(dim_in,res,split_size_h,split_size_l,num_heads_h,num_heads_l,
