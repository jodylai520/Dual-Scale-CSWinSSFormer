...
def train_epoch(model, criterion, optimizer, train_list, train_loader_args, device):
    model.train()
    running_loss = 0.0
    dataset = Synapse_dataset(train_list, RandomTransform((256, 256)))
...
def validate_epoch(model, criterion, validate_list, validate_loader_args, num_classes, device, scheduler=None):
    dataset = Synapse_dataset(validate_list, RandomTransform((256, 256)))
...
def test_epoch(model, criterion, test_list, validate_loader_args, num_classes, device):
    dataset = Synapse_dataset(test_list, RandomTransform((256, 256)))
    data_loader = DataLoader(dataset, **validate_loader_args)
    running_loss, dice_class = 0, torch.zeros(num_classes)
    length = len(data_loader)
    with torch.no_grad():
...
class Trainer_synapse():
    ...
    def test_model(self):
        ...
        validate_epoch(self.model, self.criterion, self.validate_list, self.validate_loader_args, self.num_classes,
                       self.device, self.scheduler)
        print('validate epoch success !')

    def get_test_result(self):
        test_loss, dice_ = test_epoch(self.model, self.criterion, self.test_list, self.validate_loader_args,
    ...
    def train(self, continue_train=False, test_epoch=None):
        ...
        for epoch in range(epoch_start, epoch_num):
            ...
            loss = train_epoch(self.model, self.criterion, self.optimizer, self.train_list, self.train_loader_args,
                               self.device)
            ...
            if epoch > 0 and epoch % self.eval_frequncy == 0:
                val_loss, dice_ = validate_epoch(self.model, self.criterion, self.validate_list,
                                                 self.validate_loader_args, self.num_classes, self.device,
                                                 self.scheduler)
                dice_ = list(np.asarray(dice_))
